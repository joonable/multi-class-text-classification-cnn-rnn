...
class TextCNNRNN(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        inputs = [tf.squeeze(input_, axis = [1]) for input_ in tf.split(pooled_concat, num_or_size_splits = reduced, axis=1)]
        #squeeze = removes mimensions of size 1 from the shape of a tensor
        # outputs, state = tf.nn.rnn(lstm_cell, inputs, initial_state=self._initial_state, sequence_length=self.real_len)
        outputs, state = tf.nn.static_rnn(lstm_cell, inputs, initial_state=self._initial_state, sequence_length=self.real_len)
